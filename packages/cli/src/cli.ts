...
export async function main(originalArgs: readonly string[]): Promise<void> {
  ...
  const visiblePackages = (await installedPackages()).filter(
    (name) => name.startsWith("@arkonjs/") || name === "axios",
  );
  ...
  if (args.selftest) {
    ...
    init += `
      ...
      import {
        ...
        toUtf8,
        Bech32,
      } from "@arkonjs/encoding";
      import { sha512, Bip39, Random } from "@arkonjs/crypto";
      import {
        coins,
        encodeAminoPubkey,
        encodeBech32Pubkey,
      ...
      import { Decimal } from "@arkonjs/math";
      import { assert, arrayContentEquals, sleep } from "@arkonjs/utils";
