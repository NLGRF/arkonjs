...
[![npm version](https://img.shields.io/npm/v/@arkonjs/launchpad.svg)](https://www.npmjs.com/package/@arkonjs/launchpad)
...
The basic usage of the package `@arkonjs/launchpad` contains the following:
...
```ts
import { Secp256k1HdWallet } from "@arkonjs/launchpad";
...
@arkonjs/launchpad now has a flexible
...
```ts
import { LcdClient } from "@arkonjs/launchpad";
...
```ts
import { LcdClient, setupAuthExtension } from "@arkonjs/launchpad";
...
import {
  MsgExecuteContract,
  setupWasmExtension,
} from "@arkonjs/cosmwasm-launchpad";
import {
...
```ts
import { executeKdf } from "@arkonjs/launchpad";
...
```ts
import { executeKdf, extractKdfConfiguration } from "@arkonjs/launchpad";
